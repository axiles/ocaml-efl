...
{
        Evas_Object* datetime = elm_datetime_add(Evas_Object_val(v_parent));
        if(datetime == NULL) caml_failwith("elm_datetime_add");
        return copy_Evas_Object(datetime);
}
...
PREFIX value ml_elm_datetime_value_max_get(value v_obj)
{
        ...
        CAMLlocal1(v_t);
        struct tm t;
        Eina_Bool flag = elm_datetime_value_max_get(Evas_Object_val(v_obj), &t);
        if(flag) {
                v_t = caml_alloc(1, 0);
...
PREFIX value ml_elm_datetime_value_max_set(value v_obj, value v_t)
{
        ...
        return Val_bool(elm_datetime_value_max_set(Evas_Object_val(v_obj), &t));
}
...
PREFIX value ml_elm_datetime_value_min_get(value v_obj)
{
        ...
        CAMLlocal1(v_t);
        struct tm t;
        Eina_Bool flag = elm_datetime_value_min_get(Evas_Object_val(v_obj), &t);
        if(flag) {
                v_t = caml_alloc(1, 0);
                Store_field(v_t, 0, copy_tm(t));
        } else v_t = Val_int(0);
...
{
        struct tm t = Tm_val(v_t);
        return Val_bool(elm_datetime_value_min_set(Evas_Object_val(v_obj), &t));
}

PREFIX value ml_elm_datetime_field_limit_get(value v_obj, value v_t)
{
        int min, max;
        elm_datetime_field_limit_get(Evas_Object_val(v_obj),
...
PREFIX value ml_elm_datetime_value_get(value v_obj)
{
        ...
        CAMLlocal1(v_t);
        struct tm t;
        Eina_Bool flag = elm_datetime_value_get(Evas_Object_val(v_obj), &t);
        if(flag) {
                v_t = caml_alloc(1, 0);
                Store_field(v_t, 0, copy_tm(t));
        } else v_t = Val_int(0);
...
{
        struct tm t = Tm_val(v_t);
        return Val_bool(elm_datetime_value_set(Evas_Object_val(v_obj), &t));
}
