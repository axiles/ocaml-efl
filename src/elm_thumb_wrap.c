...
{
        Evas_Object* thumb = elm_thumb_add(Evas_Object_val(v_parent));
        if(thumb == NULL) caml_failwith("elm_thumb_add");
        return copy_Evas_Object(thumb);
}
...
PREFIX value ml_elm_thumb_file_get(value v_obj)
{
        ...
        const char* file;
        const char* key;
        elm_thumb_file_get(Evas_Object_val(v_obj), &file, &key);
...
PREFIX value ml_elm_thumb_path_get(value v_obj)
{
        ...
        const char* path;
        const char* key;
        elm_thumb_path_get(Evas_Object_val(v_obj), &path, &key);
...
{
        Evas_Coord w, h;
        elm_thumb_size_get(Evas_Object_val(v_obj), &w, &h);
...
PREFIX value ml_elm_thumb_crop_align_get(value v_obj)
{
        ...
        CAMLlocal1(v);
        double x, y;
        elm_thumb_crop_align_get(Evas_Object_val(v_obj), &x, &y);
        v = caml_alloc(2, 0);
...
{
        int x;
        elm_thumb_compress_get(Evas_Object_val(v_obj), &x);
        return Val_int(x);
}
...
{
        int x;
        elm_thumb_quality_get(Evas_Object_val(v_obj), &x);
        return Val_int(x);
}
