...
inline value copy_Evas_Event_Key_Down(Evas_Event_Key_Down* info)
{
        ...
        CAMLlocal1(v);
        v = caml_alloc(8, 0);
        Store_field(v, 0, safe_copy_string(info->keyname));
        Store_field(v, 1, (value) info->modifiers);
        Store_field(v, 2, safe_copy_string(info->key));
        Store_field(v, 3, safe_copy_string(info->string));
        Store_field(v, 4, safe_copy_string(info->compose));
